...
The Unity Tool Gateway is the private loopback boundary between the UPM integration and
`dotcraft-unity`. It exposes C# Automation and the custom project tools enabled in the Editor.

```mermaid
flowchart LR
    Host[MCP client] -->|stdio| Server[dotcraft-unity mcp]
    CLI[dotcraft-unity call / exec] --> Gateway
    Server -->|loopback HTTP| Gateway[UPM Unity Tool Gateway]
...
- CLI commands exit after one operation. Gateway HTTP calls have a 65-second client deadline. Cancellation is cooperative and cannot roll back Unity work already started.
