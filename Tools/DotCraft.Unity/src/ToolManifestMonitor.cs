...
internal sealed class ToolManifestMonitor : BackgroundService
{
    ...
    private readonly UnityBackendSession _toolGatewayClient;
    ...
    public ToolManifestMonitor(
        ...
        UnityBackendSession toolGatewayClient,
    ...
    public void LoadInitialManifest() => Apply(_toolGatewayClient.ReadManifest(out _));
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                Apply(_toolGatewayClient.ReadManifest(out _));
            }
            catch (Exception ex)
            {
