...
public sealed class CliTests
{
    ...
    public async Task HelpAndVersionDoNotRequireAProject()
    {
        ...
        Assert.Equal(GatewayConstants.ProductVersion, version.Json.GetProperty("version").GetString());
        Assert.Equal(GatewayConstants.ProtocolVersion, version.Json.GetProperty("protocolVersion").GetInt32());
    ...
    public async Task ToolsDistinguishFallbackAndCachedSchemasWithoutClaimingLiveAvailability()
    {
        ...
        {
            protocolVersion = GatewayConstants.ProtocolVersion, revision = "test:1",
    ...
    [Fact]
    public async Task StatusRedactsTokenAndReportsProtocolMismatchAndDeadProcess()
    {
        using var fixture = new CliFixture();
        ...
        Assert.Contains("protocol mismatch", output.Json.GetProperty("message").GetString(), StringComparison.OrdinalIgnoreCase);
    ...
    public async Task UnavailableAndStaleAuthenticationDoNotReplay()
    {
        ...
        Assert.Contains("protocol mismatch", output.Json.GetProperty("errorMessage").GetString(), StringComparison.OrdinalIgnoreCase);
