...
Choose the workflow for the selected backend before requesting compilation or Domain Reload.
Neither workflow replays the initiating call after a disconnect.

## Attach recovery

Attach's native lifecycle observer coordinates a fresh bridge after managed Domain Reload.
The old execution/context is invalid and may report lost or unknown. Recovery restores the
connection, not the execution or its result. If recovery is unavailable or no fresh handshake
arrives, report that failure instead of repeatedly injecting or resending the mutation.

Request compilation or reload only once, after authorized source changes:
...
For reload without compilation, use `UnityEditor.EditorUtility.RequestScriptReload()` instead.
Afterward, issue a new bounded read-only probe through the same backend and selected project/PID.
The host can reconnect to the recovered bridge before this independent call. Verify that the
Editor is not compiling or importing, then inspect the Console. Compilation can fail without a
Domain Reload, so a connection or unchanged generation alone does not establish success.

`DcuLongRunningOperation`, `unity-operation.cs`, and `Wait-DotCraftUnityOperation.ps1` belong to
UPM/Gateway. Do not call them on Attach-only Editors. Work that must resume after reload needs a
persistent project integration or an external workflow with explicit checkpoints.

## Gateway durable operations

With the UPM Gateway, the durable operation file is the source of truth. The MCP or CLI call that
starts the work is not expected to remain connected. Use the following protocol only for Gateway.
...
Without MCP, use `dotcraft-unity exec --backend gateway --path <absolute-script-path> --args-file <json-file> --project-root <project> --json`. The JSON file contains the same script args described below. See [cli.md](cli.md) for CLI installation and result handling; the operation protocol and external waiter are shared by both transports.
