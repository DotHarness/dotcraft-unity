...
namespace DotCraft.Editor.Tests
{
    public sealed class ToolGatewayTests
    {
        ...
        [Test]
        public void GatewayStatusSummaryShowsRunningGateway()
        {
            var summary = McpGatewayStatusSummary.FromState(
                true,
                "http://127.0.0.1:39777/dotcraft/mcp",
                null);

            Assert.That(summary.IsVisible, Is.True);
            Assert.That(summary.IsRunning, Is.True);
            Assert.That(summary.Endpoint, Is.EqualTo("http://127.0.0.1:39777/dotcraft/mcp"));
            Assert.That(summary.Tooltip, Does.Contain("MCP Tool Gateway"));
            Assert.That(summary.Tooltip, Does.Contain(summary.Endpoint));
        }

        [Test]
        public void GatewayStatusSummaryHidesStoppedGateway()
        {
            var summary = McpGatewayStatusSummary.FromState(
                false,
                "http://127.0.0.1:39777/dotcraft/mcp",
            ...
            Assert.That(summary.IsRunning, Is.False);
            Assert.That(summary.LastError, Is.EqualTo("start failed"));
            Assert.That(summary.Tooltip, Is.Empty);
        }

        [Test]
        public void GatewayRuntimeNotifiesAfterApplyRestartAndShutdown()
        {
            var settings = DotCraftSettings.Instance;
            var originalEnabled = settings.EnableMcpGateway;
            using var server = CreateNoopLocalServer(GetFreeLoopbackPort());
            var runtime = new McpGatewayRuntime(server);
            var notifications = 0;
            void OnStatusChanged() => notifications++;

            runtime.StatusChanged += OnStatusChanged;
            try
            {
                settings.EnableMcpGateway = false;
            ...
            {
                runtime.StatusChanged -= OnStatusChanged;
                settings.EnableMcpGateway = originalEnabled;
                runtime.Shutdown();
            }
        }

        [Test]
        public void GatewayStatusBarPopupActionCanBeOverridden()
        {
            var opened = false;
            Rect capturedRect = default;
            McpGatewayStatusSummary capturedSummary = null;
            var summary = McpGatewayStatusSummary.FromState(
                true,
                "http://127.0.0.1:39777/dotcraft/mcp",
                null);

            McpGatewayStatusBarActions.OpenStatusPopupOverride = (rect, popupSummary) =>
            {
                opened = true;
                capturedRect = rect;
            ...
            try
            {
                McpGatewayStatusBarActions.OpenStatusPopup(new Rect(1, 2, 3, 4), summary);
            }
            finally
            {
                McpGatewayStatusBarActions.OpenStatusPopupOverride = null;
            }

            Assert.That(opened, Is.True);
            Assert.That(capturedRect, Is.EqualTo(new Rect(1, 2, 3, 4)));
            Assert.That(capturedSummary, Is.SameAs(summary));
        }

        [Test]
        public void GatewayStatusBarActionsCanBeOverridden()
        {
            var assistantOpened = false;
            var settingsOpened = false;
            var setupOpened = false;
            string copiedUrl = null;
            McpGatewayStatusBarActions.OpenAssistantOverride = () => assistantOpened = true;
            McpGatewayStatusBarActions.OpenSettingsOverride = () => settingsOpened = true;
            McpGatewayStatusBarActions.OpenSetupOverride = () => setupOpened = true;
            McpGatewayStatusBarActions.CopyMcpUrlOverride = url => copiedUrl = url;
            try
            {
                McpGatewayStatusBarActions.OpenAssistant();
                McpGatewayStatusBarActions.OpenSettings();
                McpGatewayStatusBarActions.OpenSetup();
                McpGatewayStatusBarActions.CopyMcpUrl("http://localhost/mcp");
            }
            finally
            {
                McpGatewayStatusBarActions.OpenAssistantOverride = null;
                McpGatewayStatusBarActions.OpenSettingsOverride = null;
                McpGatewayStatusBarActions.OpenSetupOverride = null;
                McpGatewayStatusBarActions.CopyMcpUrlOverride = null;
            }

            Assert.That(assistantOpened, Is.True);
            Assert.That(settingsOpened, Is.True);
            Assert.That(setupOpened, Is.True);
            Assert.That(copiedUrl, Is.EqualTo("http://localhost/mcp"));
        }

        [Test]
        public void GatewayStatusBarMovesLeftOfVisiblePeer()
        {
            var root = CreateStatusBarRoot(500);
            var indicator = CreateStatusBarElement(0, 30);
            var peer = CreateStatusBarElement(76, 40);
            root.Add(indicator);
            root.Add(peer);

            var offset = McpGatewayStatusBarIndicator.ResolveRightOffset(root, indicator);

            Assert.That(offset, Is.EqualTo(120));
        }

        [Test]
        public void GatewayStatusBarIgnoresHiddenPeerAndFallsBackWithoutRoot()
        {
            var root = CreateStatusBarRoot(500);
            var indicator = CreateStatusBarElement(0, 30);
            var peer = CreateStatusBarElement(76, 40);
            peer.style.display = DisplayStyle.None;
            root.Add(indicator);
            root.Add(peer);

            Assert.That(McpGatewayStatusBarIndicator.ResolveRightOffset(root, indicator), Is.EqualTo(76));
            Assert.That(McpGatewayStatusBarIndicator.ResolveRightOffset(null, indicator), Is.EqualTo(76));
        }

        [Test]
        public void GatewayStatusBarClampsOffsetToNarrowRoot()
        {
            var root = CreateStatusBarRoot(100);
            var indicator = CreateStatusBarElement(0, 30);
            root.Add(indicator);
            root.Add(CreateStatusBarElement(76, 40));

            var offset = McpGatewayStatusBarIndicator.ResolveRightOffset(root, indicator);

            Assert.That(offset, Is.EqualTo(70));
        }
        ...
        [Test]
        public void LocalServerSupportsChunkedMcpSessionFlow()
        {
            var port = GetFreeLoopbackPort();
            ToolGatewayLocalServer.ResetShutdownTokenForTests(port);
            using var server = CreateNoopLocalServer(port);
            ...
            var response = WaitForResult(SendChunkedPostAsync(
                    port,
                    ToolGatewayMcpProtocol.Paths.Mcp,
                    McpInitializeRequest().ToString(Formatting.None),
                    includeChunkExtensionAndTrailer: true),
                timeoutMilliseconds: 10000);
            var sessionId = ReadHttpHeader(response, ToolGatewayMcpProtocol.Headers.McpSessionId);
            ...
            var initialized = WaitForResult(SendChunkedPostAsync(
                    port,
                    ToolGatewayMcpProtocol.Paths.Mcp,
                    new JObject
                    {
            ...
            var tools = WaitForResult(SendChunkedPostAsync(
                    port,
                    ToolGatewayMcpProtocol.Paths.Mcp,
                    McpRequest(5, ToolGatewayMcpProtocol.Methods.ToolsList, new JObject()).ToString(Formatting.None),
                    new Dictionary<string, string>
        ...
        [TestCase("1\r\naXY", false)]
        [TestCase("4\r\nab", true)]
        public void LocalServerRejectsMalformedChunkedBodies(string framing, bool shutdownSend)
        {
            var response = SendInvalidFramedRequest(
                transferEncoding: "chunked",
                framing,
                shutdownSend: shutdownSend);

            Assert.That(response, Does.Contain("HTTP/1.1 400 Bad Request"));
        }

        [Test]
        public void LocalServerRejectsAmbiguousRequestFraming()
        {
            var response = SendInvalidFramedRequest(
                transferEncoding: "chunked",
                framing: string.Empty,
                contentLength: "0");
        ...
        [Test]
        public void LocalServerRejectsOversizedChunkedBodyBeforeReadingPayload()
        {
            var response = SendInvalidFramedRequest(
                transferEncoding: "chunked",
                framing: "400001\r\n");

            Assert.That(response, Does.Contain("HTTP/1.1 413 Payload Too Large"));
        }

        [Test]
        public void LocalServerRejectsOversizedChunkMetadata()
        {
            var response = SendInvalidFramedRequest(
                transferEncoding: "chunked",
                framing: "1;" + new string('a', 64 * 1024) + "\r\n");

            Assert.That(response, Does.Contain("HTTP/1.1 400 Bad Request"));
        }

        [Test]
        public void LocalServerRejectsUnsupportedTransferCoding()
        {
            var response = SendInvalidFramedRequest(
                transferEncoding: "gzip",
                framing: string.Empty);

            Assert.That(response, Does.Contain("HTTP/1.1 501 Not Implemented"));
        }
        ...
        private static async Task<string> SendChunkedPostAsync(
            int port,
            string target,
            string body,
            IReadOnlyDictionary<string, string> extraHeaders = null,
            bool includeChunkExtensionAndTrailer = false)
        {
            var bodyBytes = Encoding.UTF8.GetBytes(body ?? string.Empty);
            var fragments = new List<byte[]>
            {
                Encoding.ASCII.GetBytes(BuildPostHeaders(
                    port,
                    target,
                    "ChUnKeD",
                    extraHeaders,
                    includeChunkExtensionAndTrailer ? "X-DotCraft-Test" : null))
            };

            var firstLength = Math.Max(1, bodyBytes.Length / 3);
            var secondLength = Math.Max(1, (bodyBytes.Length - firstLength) / 2);
            var thirdLength = bodyBytes.Length - firstLength - secondLength;
            AddChunkFragments(
                fragments,
                bodyBytes,
                0,
                firstLength,
                includeChunkExtensionAndTrailer ? ";dotcraft=test" : string.Empty);
            AddChunkFragments(fragments, bodyBytes, firstLength, secondLength, string.Empty);
            if (thirdLength > 0)
                AddChunkFragments(fragments, bodyBytes, firstLength + secondLength, thirdLength, string.Empty);

            fragments.Add(Encoding.ASCII.GetBytes("0\r"));
            fragments.Add(Encoding.ASCII.GetBytes("\n"));
            if (includeChunkExtensionAndTrailer)
                fragments.Add(Encoding.ASCII.GetBytes("X-DotCraft-Test: accepted\r\n"));
            fragments.Add(Encoding.ASCII.GetBytes("\r\n"));

            return await SendRawHttpFragmentsAsync(port, fragments).ConfigureAwait(false);
        }

        private static string SendInvalidFramedRequest(
            ...
            bool shutdownSend = false)
        {
            ...
            var headers = BuildPostHeaders(
                port,
                ToolGatewayMcpProtocol.Paths.Mcp,
                transferEncoding,
                contentLength: contentLength);
            var response = WaitForResult(
                SendRawHttpFragmentsAsync(
                    port,
                    new[]
                    {
                        Encoding.ASCII.GetBytes(headers),
                        Encoding.UTF8.GetBytes(framing ?? string.Empty)
                    },
                    shutdownSend),
                timeoutMilliseconds: 10000);
        ...
        private static void AddChunkFragments(
            ICollection<byte[]> fragments,
            byte[] body,
            int offset,
            int count,
            string extension)
        {
            var chunkHeader = count.ToString("X") + extension + "\r\n";
            fragments.Add(Encoding.ASCII.GetBytes(chunkHeader.Substring(0, 1)));
            fragments.Add(Encoding.ASCII.GetBytes(chunkHeader.Substring(1)));

            var firstPartLength = Math.Max(1, count / 2);
            fragments.Add(CopyBytes(body, offset, firstPartLength));
            if (firstPartLength < count)
                fragments.Add(CopyBytes(body, offset + firstPartLength, count - firstPartLength));
            fragments.Add(Encoding.ASCII.GetBytes("\r"));
            fragments.Add(Encoding.ASCII.GetBytes("\n"));
        }
        ...
        private static async Task<string> SendRawHttpFragmentsAsync(
            int port,
            ...
            bool shutdownSend = false)
        {
            ...
            foreach (var fragment in fragments)
                await stream.WriteAsync(fragment, 0, fragment.Length).ConfigureAwait(false);
        ...
        private static VisualElement CreateStatusBarRoot(float width)
        {
        ...
        private static VisualElement CreateStatusBarElement(float right, float width)
        {
