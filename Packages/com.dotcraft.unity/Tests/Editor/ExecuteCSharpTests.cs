...
namespace DotCraft.Editor.Tests
{
    public sealed class ExecuteCSharpTests
    {
        ...
        [Test]
        public void ExecuteCSharpAcceptsAbsoluteSkillScriptsOutsideTheProjectRoot()
        {
            var path = Path.Combine(Path.GetTempPath(), "dotcraft-snippet-" + Guid.NewGuid().ToString("N") + ".cs");
            try
            {
                File.WriteAllText(path, "return 42;");
