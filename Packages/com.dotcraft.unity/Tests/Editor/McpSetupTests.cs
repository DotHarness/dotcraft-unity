    ...
    public sealed class McpSetupTests
    {
        ...
        public void ClaudeProviderCreatesMergesAndUninstallsProjectMcpJson()
        {
            ...
            Assert.That(root["mcpServers"]?["dotcraft-unity"]?["args"]?.Values<string>(),
                Is.EqualTo(new[] { "mcp", "--backend", "gateway", "--project-root", _tempRoot }));
        ...
        public void CodexProviderReplacesManagedBlockAndPreservesUnrelatedToml()
        {
            ...
            Assert.That(toml, Does.Contain("args = [\"mcp\", \"--backend\", \"gateway\", \"--project-root\""));
        ...
        [Test]
        public async Task AgentSkillInstallerInstallsFetchedSkillFiles()
        {
            var installer = SkillInstaller(
                SkillFile("SKILL.md", "---\nname: dotcraft-unity\ndescription: Test\n---\n"),
                SkillFile("references/api.md", "# API\n"));

            var result = await installer.InstallAsync(_tempRoot, ".agents/skills/dotcraft-unity");
        ...
        [Test]
        public async Task AgentSkillInstallerIsIdempotentWhenFilesMatch()
        {
            var installer = SkillInstaller(SkillFile("SKILL.md", "---\nname: dotcraft-unity\ndescription: Test\n---\n"));

            var first = await installer.InstallAsync(_tempRoot, ".agents/skills/dotcraft-unity");
            var second = await installer.InstallAsync(_tempRoot, ".agents/skills/dotcraft-unity");
        ...
        [Test]
        public async Task AgentSkillInstallerReplacesExistingChangedSkill()
        {
        ...
        [Test]
        public async Task AgentSkillInstallerRejectsPathsOutsideProjectRoot()
        {
            var installer = SkillInstaller(SkillFile("SKILL.md", "test"));

            var result = await installer.InstallAsync(_tempRoot, "../dotcraft-unity-skill");
        ...
        [Test]
        public async Task AgentSkillInstallerRequiresSkillMarkdown()
        {
        ...
        [Test]
        public async Task ReleaseSkillArchiveInstallsOnlySkillFiles()
        {
            var files = ReleaseAgentSkillSourceFetcher.ReadArchive(SkillArchive(
                SkillFile(".craft-plugin/plugin.json", "{}"),
                SkillFile("skills/dotcraft-unity/SKILL.md", "release skill"),
                SkillFile("skills/dotcraft-unity/references/api.md", "API")));
            var result = await new AgentSkillInstaller(new MemorySkillSourceFetcher(files))
                .InstallAsync(_tempRoot, ".agents/skills/dotcraft-unity");

            Assert.That(result.Success, Is.True, result.Error);
            Assert.That(File.ReadAllText(Path.Combine(result.Path, "SKILL.md")), Is.EqualTo("release skill"));
            Assert.That(File.ReadAllText(Path.Combine(result.Path, "references", "api.md")), Is.EqualTo("API"));
            Assert.That(files.Count, Is.EqualTo(2));
        }

        [Test]
        public async Task ReleaseSkillArchiveRejectsTraversalWithoutReplacingExistingSkill()
        {
            var target = Path.Combine(_tempRoot, "skill");
            Directory.CreateDirectory(target);
            File.WriteAllText(Path.Combine(target, "SKILL.md"), "existing");
            var files = ReleaseAgentSkillSourceFetcher.ReadArchive(SkillArchive(
                SkillFile("skills/dotcraft-unity/SKILL.md", "replacement"),
                SkillFile("skills/dotcraft-unity/../outside.txt", "escape")));
            var result = await new AgentSkillInstaller(new MemorySkillSourceFetcher(files))
                .InstallAsync(_tempRoot, "skill");

            Assert.That(result.Success, Is.False);
            Assert.That(File.ReadAllText(Path.Combine(target, "SKILL.md")), Is.EqualTo("existing"));
            Assert.That(File.Exists(Path.Combine(_tempRoot, "outside.txt")), Is.False);
        }

        private static byte[] SkillArchive(params AgentSkillFile[] files)
        {
            ...
            using (var zip = new ZipArchive(stream, ZipArchiveMode.Create, leaveOpen: true))
            {
                ...
                {
                    using var target = zip.CreateEntry(file.RelativePath).Open();
        ...
        private sealed class MemorySkillSourceFetcher : IAgentSkillSourceFetcher
        {
            ...
            public Task<IReadOnlyList<AgentSkillFile>> FetchAsync() => Task.FromResult(_files);
        }
    }
}
