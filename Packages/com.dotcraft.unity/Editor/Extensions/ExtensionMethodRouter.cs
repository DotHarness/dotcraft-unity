...
namespace DotCraft.Editor.Extensions
{
    ...
    {
        private readonly ConcurrentDictionary<string, Func<JToken, CancellationToken, Task<object>>> _handlers = new();
        ...
        {
            _handlers[method] = (parameters, _) => handler(parameters);
        }
        ...
        {
            _handlers[method] = (paramsJson, _) => Task.FromResult(handler(paramsJson));
        }
        ...
        public async Task<object> HandleAsync(string method, JToken paramsJson)
        {
            ...
            return await MainThreadDispatcher.RunOnMainThread(token => handler(paramsJson, token));
        }
        ...
        internal void RegisterRuntimeTools(IEnumerable<RuntimeToolDefinition> runtimeTools)
        {
            ...
            foreach (var runtimeTool in runtimeTools)
            {
                ...
                _handlers[acpMethod] = (paramsJson, token) => InvokeRuntimeToolAsync(runtimeTool, paramsJson, token);
