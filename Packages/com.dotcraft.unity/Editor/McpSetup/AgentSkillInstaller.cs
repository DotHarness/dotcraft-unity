...
namespace DotCraft.Editor.McpSetup
{
    ...
    {
        Task<IReadOnlyList<AgentSkillFile>> FetchAsync();
    }
    ...
    internal sealed class AgentSkillInstaller
    {
        ...
        public static AgentSkillInstaller CreateDefault() =>
            new(new ReleaseAgentSkillSourceFetcher());

        public async Task<AgentSkillInstallResult> InstallAsync(string projectRoot, string skillRelativePath)
        {
            ...
            try
            {
                ...
                var files = await _fetcher.FetchAsync();
