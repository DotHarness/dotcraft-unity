    ...
    internal sealed class McpGatewaySetupWindow : EditorWindow
    {
        ...
        private VisualElement BuildGatewayCard()
        {
            ...
            card.Add(GatewayPanelView.KeyValueRow(
                ...
                string.Empty,
                out _gatewayValue));
            card.Add(GatewayPanelView.KeyValueRow("Manifest", string.Empty, out _manifestValue));
            card.Add(GatewayPanelView.KeyValueRow("Tools", string.Empty, out _toolsValue));
            ...
            _installGatewayButton = GatewayPanelView.Button("Install CLI", InstallGateway);
        ...
        private async void InstallClient(ClientCardView view)
        {
            ...
            if (result.Success)
                skillResult = await SkillInstaller.InstallAsync(McpGatewaySetupDefaults.ProjectRoot, view.Provider.SkillRelativePath);
        ...
        private void RefreshGatewayStatus()
        {
            ...
            _installGatewayButton.text = McpGatewayInstaller.HasExecutable ? "Update CLI" : "Install CLI";
            _gatewayValue.text = gateway.IsInstalled
                ? $"{gateway.Version} · protocol compatible"
