...
`unity_execute_csharp` compiles and executes optional leading `using` directives followed by C# method-body statements in the running Unity Editor process. Leading ordinary, alias, `static`, and `global` using directives are supported. `using var` declarations and `using (...)` statements remain in the method body. Namespace, class, and method declarations are not accepted as snippet input.
